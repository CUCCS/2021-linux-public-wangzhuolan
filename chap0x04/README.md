...
- 提前在终端安装`imagemagick`和`shellcheck`
    ```bash
    sudo apt-get update
    ...
    ```
- 在vscode上安装扩展
  ![vscode扩展](img/vscode-extension.png)
- 然后编写脚本进行并运行
- 任务一实现结果
  ![任务一实现情况](img/task1-success.png)

#### 任务一遇到的问题：
+ 由于是在师哥/姐的仓库代码基础上完成自己的实验，所以有许多问题与哥姐的问题重合性很高，或者提前看了哥姐遇到的坑，避免了许多问题的出现。
+ 刚开始运行的时候没有输出成功的提示信息，所以进行逐步调试，发现文件的后缀名写错了，应该是jpeg，我写的是jpg。

---

**2. 任务二**
+ 实现结果
![任务一实现情况](img/task2-success.png)

--- 
**3. 任务三**
+ 实现结果
```
cuc@wang:~$ bash task3.sh -h
doc
...
```
```
cuc@wang:~$ bash task3.sh -s /images/NASA-logosmall.gif
...
+ [师哥/姐的仓库](https://github.com/CUCCS/linux-2020-LyuLumos/tree/ch0x04/ch0x04)
